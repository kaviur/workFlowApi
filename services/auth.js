...
class Auth{
    ...
    async login(email,password){
        ...
        if(user){
            ...
            }//else{}"El usuario y la contraseña no coinciden"
        }
        return {success:false,message:"El correo ingresado no se encuentra registrado"}
    }

    async signup(userData){
        if(await this.users.getByEmail(userData.email)){
            return {succes:false,message:"La cuenta de correo ya se encuentra registrada"}
        }else{
            userData.role = 0
